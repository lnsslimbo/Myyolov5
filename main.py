...
class CamConfig:
    ...
    def show_pic(self):
        # 全局变量
        # 在函数中引入定义的全局变量
        global EYE_AR_THRESH, EYE_AR_CONSEC_FRAMES, MAR_THRESH, MOUTH_AR_CONSEC_FRAMES, HAR_THRESH, NOD_AR_CONSEC_FRAMES, COUNTER, TOTAL, mCOUNTER, mTOTAL, hCOUNTER, hTOTAL, ActionCOUNTER, Roll, Rolleye, Rollmouth
        ...
        if success:
            ...
            if head > HAR_THRESH:
                ...
                if hCOUNTER >= NOD_AR_CONSEC_FRAMES:
                    hTOTAL += 1
                    # window.label_4.setText("点头次数：" + str(hTOTAL))
                # 重置点头帧计数器
                hCOUNTER = 0
